...
class List2D(object):
    ...
    def abs(self) -> List2D:
        """Return a new List2D object with the absolute value of each element."""
        vals = [
            [abs(self.vals[i][j]) for j in range(self.ncol)] for i in range(self.nrow)
        ]
    ...
    def leakyrelu(self) -> List2D:
        """Return a new List2D object with the LeakyReLU of each element."""
        vals = [
            [
                self.vals[i][j] * (0.01 if self.vals[i][j] <= 0 else 1)
